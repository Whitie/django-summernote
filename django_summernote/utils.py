...
SUMMERNOTE_THEME_FILES = {
    "bs3": {
        "base_css": ("summernote/bootstrap.min.css",),
        "base_js": (
    ...
    "bs4": {
        "base_css": (
            "https://cdn.jsdelivr.net/npm/bootstrap@4.6.2/dist/css/"
            "bootstrap.min.css",
        ),
        "base_js": (
            "summernote/jquery-3.6.0.min.js",
            "https://cdn.jsdelivr.net/npm/bootstrap@4.6.2/dist/js/"
    ...
    "lite": {
        "base_css": (),
        "base_js": ("summernote/jquery-3.6.0.min.js",),
        "default_css": (
    ...
    try:
        ...
        if not issubclass(klass, AbstractAttachment):
            raise ImproperlyConfigured(
                "SUMMERNOTE_CONFIG['attachment_model'] refers to "
                "model '%s' that is not "
                "inherited from 'django_summernote.models.AbstractAttachment'"
                % config["attachment_model"]
            )
        return klass
    except ValueError:
        raise ImproperlyConfigured(
            "SUMMERNOTE_CONFIG['attachment_model'] must be of the "
...
def get_attachment_storage():
    ...
    if config["attachment_storage_class"]:
        ...
        try:
            ...
            raise ImproperlyConfigured(
                "%s is not a valid module" % storage_path
            )

        storage_module, storage_classname = (
            storage_path[:dot],
            storage_path[dot + 1 :],  # noqa: E203
        )
        ...
        try:
            ...
            raise ImproperlyConfigured(
                'Error importing storage module %s: "%s"' % (storage_module, e)
            )
...
def has_codemirror_config():
    config = get_config()
    return "summernote" in config and "codemirror" in config["summernote"]
