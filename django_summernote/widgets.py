...
class SummernoteWidgetBase(forms.Textarea):
    ...
    def render(self, name, value, attrs=None, **kwargs):
        ...
        return super().render(name, value, attrs=attrs_for_textarea, **kwargs)
...
class SummernoteWidget(SummernoteWidgetBase):
    def render(self, name, value, attrs=None, **kwargs):
        ...
        context = {
            ...
            # Width and height have to be pulled out to create an
            # iframe with correct size
            "width": summernote_settings["width"],
            "height": summernote_settings["height"],
        }

        html += render_to_string(
            "django_summernote/widget_iframe.html", context
        )
...
class SummernoteInplaceWidget(SummernoteWidgetBase):
    def _media(self):
        ...
        return forms.Media(
            css={
                "all": (
                    (
                        config["codemirror_css"]
                        if has_codemirror_config()
                        else ()
                    )
                    + config["default_css"]
    ...
    def render(self, name, value, attrs=None, **kwargs):
        ...
        html += render_to_string(
            "django_summernote/widget_inplace.html", context
        )
        return mark_safe(html)
    ...
    def final_attr(self, attr):
        ...
        if "class" in attrs_for_final:
            attrs_for_final["class"] = attrs_for_final["class"].replace(
                " form-control", ""
            )
        return attrs_for_final
